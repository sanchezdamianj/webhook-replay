...
export function webhookRouter(pool) {
    const router = Router();
    router.post("/webhook/:id", async (req, res, next) => {
        const { id } = req.params;
        try {
            const destinationResult = await pool.query("select id, public_key from destinations where public_key = $1 limit 1", [id]);
            ...
            if (idempotencyKey) {
                const existing = await pool.query("select id from webhook_events where destination_id = $1 and idempotency_key = $2 limit 1", [destination.id, idempotencyKey]);

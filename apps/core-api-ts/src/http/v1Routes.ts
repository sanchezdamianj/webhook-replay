...
export type V1Deps = {
  ...
  logExportQueue: Queue<LogExportJobData, LogExportJobResult, string>;
  redis: Redis;
};
...
async function authenticate(
  ...
  let payload: { user_id: number; account_id: number };
  try {
...
export async function registerV1Routes(app: FastifyInstance, deps: V1Deps) {
  ...
  app.patch("/v1/me/ui_flags", async (req, reply) => {
    ...
    const prev = (ctx.user.uiFlags as Record<string, unknown>) ?? {};
    const nextFlags = { ...prev, ...(incoming as Record<string, unknown>) };
  ...
  app.post("/v1/destinations", async (req, reply) => {
    ...
    const dest = (req.body as { destination?: { name?: string; target_url?: string } })?.destination;
  ...
  app.patch("/v1/destinations/:id", async (req, reply) => {
    ...
    if (!ctx) return;
    const id = Number((req.params as { id: string }).id);
    const destBody = (req.body as { destination?: { name?: string; target_url?: string } })?.destination;
  ...
  app.get("/v1/events", async (req, reply) => {
    ...
    const q = req.query as { page?: string; destination_id?: string };
    ...
    const latestByEvent = new Map<number, (typeof deliveryAttempts.$inferSelect)>();
    ...
    if (eventIds.length > 0) {
      ...
      for (const a of attemptRows) {
        if (!latestByEvent.has(a.webhookEventId)) latestByEvent.set(a.webhookEventId, a);
      }
    }
  ...
  app.post("/v1/events/:event_id/replay", async (req, reply) => {
    const ctx = await authenticate(req, reply, deps);
    if (!ctx) return;
    const eventId = Number((req.params as { event_id: string }).event_id);
  ...
  app.get("/v1/delivery_attempts", async (req, reply) => {
    ...
    const eventIdRaw = (req.query as { event_id?: string }).event_id;
    const eventId =
      eventIdRaw != null && eventIdRaw !== "" ? Number(eventIdRaw) : null;
    ...
    const attempts =
      eventId != null
        ? await db
            ...
            .innerJoin(destinations, eq(webhookEvents.destinationId, destinations.id))
            .where(
              and(eq(destinations.accountId, ctx.account.id), eq(deliveryAttempts.webhookEventId, eventId))
            )
            .orderBy(desc(deliveryAttempts.requestedAt))
            .limit(100)
        : await db
  ...
  app.post("/v1/exports/logs", async (req, reply) => {
    ...
    const destinationId =
      destinationIdRaw == null || destinationIdRaw === ""
        ? null
    ...
    const job = await logExportQueue.add(
      "export_logs",
      {
        accountId: ctx.account.id,
  ...
  app.get("/v1/exports/logs/:id", async (req, reply) => {
    ...
    const data = job.data as LogExportJobData;
    ...
    const result = (job.returnvalue ?? null) as LogExportJobResult | null;
  ...
  app.get("/v1/exports/logs/:id/download", async (req, reply) => {
    ...
    const result = job.returnvalue as LogExportJobResult | undefined;
    if (!result?.filePath) {
    ...
    stream.on("error", async () => {
      if (!reply.sent) await reply.code(404).send({ error: "not_found" });
    });
